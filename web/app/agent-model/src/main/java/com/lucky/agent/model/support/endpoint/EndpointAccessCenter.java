...
public class EndpointAccessCenter {
    ...
    /** 将端点配置适配为 LangChain4j ChatLanguageModel（携带当前推理深度与用量统计）。 */
    public ModelEndpoint toEndpoint(ModelConfig config) {
        return new ModelEndpointImpl(config, healthProbe, inferenceDepth, usageTracker);
    }

    /** 指定端点用量统计（无记录返回 null）。 */
    public ModelUsageTracker.ModelUsage usageOf(String id) {
        return usageTracker.usage(id);
    }

    /** 全部端点用量统计（模型详情小窗数据源）。 */
    public Map<String, ModelUsageTracker.ModelUsage> usages() {
