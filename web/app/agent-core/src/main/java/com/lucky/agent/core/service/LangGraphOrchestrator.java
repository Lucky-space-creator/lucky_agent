...
/**
 * LangGraph4j 编排层：与 {@code thin} 主循环<b>共用同一套运行时</b>的图实现。
 *
 * <p>本类保留 LangGraph4j 的差异价值——用显式状态图表达「分析 → 拆解判定 → 执行 → 复查 → 记忆 → 总结」
 * 的拓扑与条件路由；但把<b>判定/验证/记忆/预算/韧性</b>这些会随需求演进的逻辑全部委托给共享运行时，
 * 从而消除双主循环漂移：</p>
 *
 * <table border="1">
 *   <caption>共享运行时对应关系</caption>
 *   <tr><th>原本类内部实现</th><th>现委托给</th></tr>
 *   <tr><td>{@code engine.run(...)} 直接调用</td><td>{@link ModelGateway}（重试退避 + 熔断）</td></tr>
 *   <tr><td>{@code verificationChain.verify(...)}</td><td>{@link LoopController} → 运行时验证链（客观优先）</td></tr>
 *   <tr><td>{@code earlyStopPolicy.shouldStop(...)}</td><td>{@link LoopController}（同一份早停语义）</td></tr>
 *   <tr><td>{@code loopMemoryManager.manage(...)}</td><td>{@link LoopController} → {@code MemoryPort}（成功才沉淀/失败只压缩）</td></tr>
 *   <tr><td>{@code evidenceConfidence(...)} 自算通过率</td><td>{@code ChainVerifierAdapter}（置信度单一事实源）</td></tr>
 *   <tr><td>{@code RunBudget} 自建预算</td><td>{@link RuntimeSessionFactory} 的全局四级预算</td></tr>
 * </table>
 *
 ...
 *             ├ 达成 → summarize
 *             └ 未达成 → memory(K: 推进目标) → 回到 analyze（外层 for 下一轮）
 *   （N: 安全阀 迭代 / 回合 / token 预算 → 强制结束 → summarize）
 *   → summarize(P: 总结) → END(Q: 输出)
 * </pre>
 *
 * <p>关键判断通过 {@link AgentEvent#thought} 推送到思考区展示；引擎在编排模式下不逐次发布
...
public class LangGraphOrchestrator implements AgentOrchestrator {
    ...
    public LangGraphOrchestrator(ModelGateway modelGateway, ConversationStateManager stateManager,
                                 PlanGenerator planGenerator, PlanValidator planValidator,
                                 Replanner replanner, TaskProgressTracker coreTaskProgressTracker,
                                 AgentRuntime runtime, LoopController loopController,
                                 RuntimeSessionFactory sessions, CoreProperties properties,
                                 AskSuspender askSuspender, ActScheduler actScheduler) {
    ...
    /**
     * 用 LangGraph 状态图运行一次编排（复杂/简单任务统一入口）。
     *
     * @param ref       会话
    ...
    public EngineRunResult run(SessionRef ref, ConversationCtx ctx, AgentEventPublisher publisher) {
        ...
        try {
            return runGraph(ref, ctx, publisher, session, rc);
        } finally {
            sessions.close(session, runtime);
        }
    }

    private EngineRunResult runGraph(SessionRef ref, ConversationCtx ctx, AgentEventPublisher publisher,
                                    RuntimeSession session, RuntimeContext rc) {
        String sessionId = ref.sessionId();
        ...
        for (int iter = 1; ; iter++) {
            ...
            EngineRunResult tripped = checkSafetyValve(sessionId, iter, maxIter, turns, session,
            ...
            // 与 thin 主循环共用同一套中间件前置钩子（预算熔断、记忆召回、日志等）
            if (!runtime.beforeLlm(session.loopContext())) {
            ...
            try {
                StateGraph<AgentState> graph = new StateGraph<>(
                        ...
                        .addNode("singleAct", node((s) -> singleAct(s, ctx, publisher, rc)))
                        .addNode("execute", node((s) -> execute(s, ctx, stateManager.session(ref), publisher)))
                        .addNode("review", node((s) -> review(s, ctx, publisher, rc)))
                        .addNode(NODE_MEMORY, node((s) -> memory(s, ctx, publisher)))
            ...
            // 预算累计：节点内不直接记预算，由外层统一累计到全局作用域（token 自底向上）
            Object tok = finalState.value(K_TOKENS).orElse(0L);
            if (tok instanceof Number n) {
                rc.budget().record(null, n.longValue());
            }
            ...
            if (Boolean.TRUE.equals(finalState.value(K_ABORTED).orElse(false))) {
                ...
                return EngineRunResult.of(sessionId, Phase.ACT, summary,
                        session.global().usedTokens(), null, "aborted");
            }
            ...
            if (Boolean.TRUE.equals(finalState.value(K_DONE).orElse(false))) {
                ...
                return EngineRunResult.of(sessionId, Phase.ACT, summary,
                        session.global().usedTokens(), null, "success");
            }

            // J=未达成且未超上限 → memory 节点已写入 continueGoal，回 analyze（外层 for 下一轮）
            accumulated.append(asStr(finalState.value(K_LOG), ""));
    ...
    private Map<String, Object> singleAct(AgentState s, ConversationCtx ctx, AgentEventPublisher publisher,
        ...
        int iter = asInt(s.value(K_ITER), 1);
        publisher.publish(sessionId, AgentEvent.thought(sessionId,
        ...
        if (direct.status() != null && direct.status().equals("ask")) {
            ...
            askSuspender.suspend(ctx.sessionRef(), askReason(ctx), ctx.permissionLevel() == null
        ...
        // P2-2：单步任务也走验证（无客观声明时降级为主观判定）——判定/记忆统一由 LoopController 承担
        String output = safeText(direct.finalText());
        LoopDecision decision = loopController.decide(iter, goal, output, rc, runtime);
        out.put(K_LOG, "\n- 直接执行：" + output);
        out.put(K_TOKENS, direct.tokenUsed());
        if (decision.terminate()) {
            if (decision.isAsk()) {
                askSuspender.suspend(ctx.sessionRef(), decision.text(), "low_confidence");
                out.put(K_ASK, true);
                out.put(K_ASK_RESULT, EngineRunResult.of(sessionId, Phase.ACT, decision.text(), 0, null, "ask"));
            } else {
                out.put(K_DONE, true);
                out.put(K_SUMMARY, decision.text());
            }
            return out;
        }
        out.put(K_REASON, "单步执行未完全达成");
        out.put(K_CONTINUE_GOAL, decision.nextGoal());
        return out;
    }
    ...
    private Map<String, Object> execute(AgentState s, ConversationCtx ctx,
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int attempt = 0; attempt <= maxRetry; attempt++) {
                ...
                r = modelGateway.call(withSuppress(ctx, Phase.ACT, t.title()), Phase.ACT, t.title());
    ...
    /**
     * D/I: 客观验证 + 达成度判定（客观证据优先于主观结论）+ K 记忆管理。
     *
     * <p>判定、卡死守卫、早停与记忆全部委托给 {@link LoopController}——与 thin 主循环同一份实现，
     * 因此「验证怎么算通过、什么时候该停、成功/失败如何写记忆」在全项目只有一处定义。</p>
     */
    private Map<String, Object> review(AgentState s, ConversationCtx ctx, AgentEventPublisher publisher,
                                      RuntimeContext rc) {
        String sessionId = ctx.sessionId();
        String goal = asStr(s.value(K_GOAL), ctx.goal());
        ...
        Plan plan = (Plan) s.value(K_PLAN).orElse(null);
        // 传入真实计划：客观验证按计划内声明的 verify 逐步骤执行
        LoopDecision decision = loopController.decide(iter, goal, execLog, rc, runtime, plan);
        out.put(K_TOKENS, 0L);
    ...
    /** K: 记忆推进（压缩/沉淀已在 review 内经 LoopController 完成，此处仅推进 goal）。 */
    private Map<String, Object> memory(AgentState s, ConversationCtx ctx, AgentEventPublisher publisher) {
        String sessionId = ctx.sessionId();
        Map<String, Object> out = new HashMap<>();
    ...
    private EngineRunResult checkSafetyValve(String sessionId, int iter, int maxIter, int turns,
                                             RuntimeSession session, StringBuilder accumulated,
                                             AgentEventPublisher publisher) {
        String reason = null;
        String detail = null;
        if (iter > maxIter) {
        ...
        return EngineRunResult.of(sessionId, Phase.ACT, summary, session.global().usedTokens(), null, reason);
    }
    ...
    private Plan analyzePlan(ConversationCtx ctx, String goal, AgentEventPublisher publisher) {
        EngineRunResult planResult = modelGateway.call(withSuppress(ctx, Phase.PLAN, goal), Phase.PLAN, goal);
