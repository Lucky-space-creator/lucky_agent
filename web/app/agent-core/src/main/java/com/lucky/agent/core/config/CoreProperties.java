 ...
 * @param orchestratorMode          核心编排器实现选择：reactor（默认，REACT 主回环）/ langgraph（LangGraph4j 状态图）/ thin（薄主循环 + 厚运行时）
